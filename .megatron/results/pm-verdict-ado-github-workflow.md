...
### Key Agreements
- The **strategic goal is correct**: Megatron must support ADO alongside GitHub to widen its user base and fulfill the multi-platform orchestrator vision.
...
2. **Platform detector**: Create `src/utils/vcsDetect.ts` that parses `git remote get-url origin` and returns `{ platform: 'github' | 'ado', owner, repo, project? }`. Add configurable override via `.megatron/config.json` → `"vcs_provider"` for dual-remote repos.
