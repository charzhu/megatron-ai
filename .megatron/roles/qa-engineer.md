...
For ANY PR that modifies `init.js`, `upgrade.js`, or any code that writes to `.megatron/config/`:
- Verify that running `megatron upgrade` on a workspace with customized vcs.json preserves all user values
- Verify that running `megatron init` on an existing workspace does not overwrite user config
