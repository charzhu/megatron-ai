...
Enables plan-mode agents to write files to `.megatron/` only. Two-layer security:
1. Lexical `startsWith(megatronRoot + path.sep)` — prevents `..` and sibling bypass
...
`megatron upgrade` force-overwrote `.megatron/config/vcs.json`, wiping user's ADO organization and project values.
