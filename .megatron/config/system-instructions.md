...
## Artifact Isolation
ALL generated reports, tasks, and memory artifacts MUST be saved inside `.megatron/` subdirectories. Never write loose files to the repository root.

## Workflow
1. **Issue First** — Create a GitHub Issue via MCP
2. **Analyze & Bind** — Create `.megatron/tasks/task_issue_<ID>.md`
...
Roles are strictly bounded within the Megatron Swarm to prevent hallucinations:
...
| **T2** | `.megatron/roles/<name>.md` | Role template with engine/model binding | Auto-precipitated on first delegation, Master can evolve |
...
Skills are domain-specific instruction manuals stored at `.megatron/skills/<name>/SKILL.md`.
...
- **Skill** = HOW to do the work (operational SOP, workflow, tools) — stored in `.megatron/skills/`
...
- The agent **cannot** write to source code files — only to `.megatron/` artifacts via `write_blackboard_artifact`.
...
- Tracked via `MEGATRON_DELEGATION_DEPTH` environment variable, automatically injected and incremented.
...
The system automatically injects `MEGATRON_PARENT_ISSUE` into child agent processes, allowing child-created Issues to display their parent relationship. But this only works if the orchestrating agent passes `parent_issue_number` in every delegation call.
...
## GitHub Auto-Tagging
All Issues and PRs created via MCP tools are automatically tagged with `[Megatron]` prefix and `megatron-bot` label for traceability.
...
- Prefer actionable error messages: "Auto-detect failed: git not found in PATH. Set organization and project in .megatron/config/vcs.json"
...
- The system enforces a physical build gate on `vcs_merge_pr` when `pre_merge_build.enabled` is set in `.megatron/config/vcs.json`
...
- Agents MUST NOT autonomously modify their own Role templates (`.megatron/roles/`)
...
# Part 2: Project-Specific Constraints (Megatron AI Repository)

> These rules are specific to the `megatron-ai` repository itself. They do NOT ship to end-users via `megatron init`.
...
| Layer | Path | Purpose |
|-------|------|---------|
| **Host project** | Root (`src/`, `docs/`, `.megatron/`) | The Megatron orchestrator's own development workspace |
| **Plugin package** | `megatron-plugin/` | The npm-publishable MCP server plugin that ships to end-users |

## Development & Reload Constraints (Hard Rule)
When making any code modifications to the Megatron project itself (e.g., `src/`, `megatron-plugin/`, or MCP server logic):
1. **Agent MUST Build**: The agent must automatically run the build command (`cd megatron-plugin && npm run build`) after modifications.
...
| Change Type | Apply to `.megatron/` (host) | Also apply to `megatron-plugin/` (packaging) |
|---|---|---|
| System instructions update | ✅ `.megatron/config/system-instructions.md` | ✅ `megatron-plugin/scaffold/config/system-instructions.md` |
| New/updated skill | ✅ `.megatron/skills/<name>/SKILL.md` | ✅ `megatron-plugin/skills/<name>/SKILL.md` |
| Config change (`available-agents.json`) | ✅ `.megatron/config/` | ✅ `megatron-plugin/scaffold/config/` |
| New T2 role (project-specific, e.g., `marketing`) | ✅ `.megatron/roles/` | ❌ NOT packaged — project-specific |
| T1 agent instance | ✅ `.megatron/agents/` | ❌ NEVER packaged — instance state |
| MCP server code change | N/A | ✅ `src/mcp/` → `megatron-plugin/dist/` (rebuild required) |
| init.js / CLI change | N/A | ✅ `megatron-plugin/bin/` |

### Build & Publish Checklist
After modifying plugin-relevant files:
1. `cd megatron-plugin && npm run build` — rebuild `dist/mcp-server.js`
2. Verify `megatron-plugin/scaffold/` contains the latest config and instructions
3. Verify `megatron-plugin/skills/` contains only universal bootstrap skills (not project-specific ones)
4. `git push origin master` — end-users pull via `npx -y github:cloga/megatron-ai`

### What MUST NOT Ship in the Plugin
- `.megatron/roles/` — Project-specific T2 role templates (auto-generated at runtime)
- `.megatron/agents/` — T1 instance snapshots (workspace-local)
- `.megatron/state/` — Task manifests, T3 usage logs
- `.megatron/reports/`, `.megatron/reviews/` — Generated artifacts
