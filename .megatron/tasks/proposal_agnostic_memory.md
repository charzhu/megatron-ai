...
1. **Instruction Location**: Where and how should we store the master instructions inside the \.megatron/\ directory so that it is agnostic to the LLM UI client (Claude Code, Copilot, Cursor)? 
...
3. **Continuous Evolution (Memory)**: How should we structure \.megatron/knowledge/\ or \.megatron/memory/\ to automatically capture project experience and integrate it into the global agent workflow?

Please provide a technical design for the \.megatron\ scaffolding and the injection mechanism.