...
1. **Extend Role Frontmatter**:
   Add a skills array constraint to the YAML frontmatter of T2 Role templates (megatron-plugin/roles/*.md):
   \\\yaml
   role: dev
   tier: T1
   skills:
...
3. **Prompt Injection**:
   For each skill listed, read the corresponding megatron-plugin/skills/<skill-name>/SKILL.md (or similar) and append it as \[SYSTEM: INJECTED SKILL - <name>]\ at the bottom of the System Prompt.
