---
name: delegate-task
description: Dispatches a task to a specialized agent role using async-first, non-blocking delegation via the Megatron Swarm.
---

# Delegate Task (The Megatron Dispatch)
...
## The Megatron Swarm Pre-Dispatch Doctrine
...
**Action:** 
1. If delegating to an existing T2 role (e.g., `.megatron/roles/security.md`), **read the role file first**.
2. If the current description lacks specific context, rules, or domain knowledge required for the current task (e.g., it only says "Security Engineer" but lacks ADO integration context), you **MUST** physically edit and enrich the `.megatron/roles/<name>.md` file using an edit tool before dispatching.
...
```json
{
  ...
  "output_path": ".megatron/reports/security-audit.md",
