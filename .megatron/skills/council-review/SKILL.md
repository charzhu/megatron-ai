...
3. Write this design to the Blackboard with a unique name: `.megatron/proposals/PROPOSAL_<task_topic>.md`.
...
5. The status tool will return a precise folder path matching the isolated execution timestamp (e.g., `.megatron/reviews/<timestamp>/`).
...
- **If there are NO blockers**: Implement the suggestions and output the final `.megatron/TODO.md` file (the implementation backlog).
- **If there are FATAL conflicts**: Create `.megatron/CONFLICTS.md` outlining the opposing viewpoints cleanly, pause, and ask the User to arbitrate.
