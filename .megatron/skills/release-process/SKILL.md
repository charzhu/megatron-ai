...
description: 'Standard operating procedure for releasing a new version of the Megatron project. Covers version bumping, changelog, build, documentation verification, cleanup, tagging, and pushing.'
version: 1.0.0
---
...
1. Is there a release readiness report in `.megatron/reports/`? Read it first.
...
- Read current version from `package.json` (root) and `megatron-plugin/package.json`
...
- Update `version` in `megatron-plugin/package.json`
- Both MUST match

### Step 4: Build
- Run `cd megatron-plugin && npm run build`
...
- Stage: `git add package.json megatron-plugin/package.json CHANGELOG.md megatron-plugin/dist/`
- Commit: `git commit -m "chore(release): v<VERSION>"`
- Do NOT include .megatron/ runtime files in the commit
