...
When multiple autonomous agents (`dev`, `qa-engineer`) are executed concurrently by the Master Orchestrator via the Megatron Swarm, they share the same physical filesystem path (`c:\Users\lochen\megatron-ai`). If they attempt to checkout different branches or mutate code simultaneously, Git state will conflict, the TS build will break, and tasks will suffer from race conditions.
...
2. **Agent Sandboxes:** The `Master Agent` will explicitly use `git worktree add ../megatron-isolated-task-<id> -b feat/task-<id>` to mount a new workspace folder seamlessly.
...
## 4. Git Issue Link
*This proposal is tracked for human visibility at: https://github.com/cloga/megatron-ai/issues/5*
