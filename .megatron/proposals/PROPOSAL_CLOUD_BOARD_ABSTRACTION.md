...
Instead of exposing GitHub-specific MCP tools to the Agents, we expose a **Cloud Board Interface**. The MCP Server routes these abstract commands to the active Provider (configured in `.megatron/config/`).
...
- **Local Blackboard** (`.megatron/` SQLite or files) is the high-bandwidth, high-frequency *Subconscious Memory* of the project (ephemeral task states, agent memory, deep reasoning logs).
