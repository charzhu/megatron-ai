# PROPOSAL: Clean up and Restructure the `.megatron/` Blackboard Directory

## Current State
The `.megatron/` directory acts as our local blackboard and artifact sandbox. However, it has become cluttered with loose files at its root level:
...
1. A formalized taxonomy for the `.megatron/` folder (e.g., should proposals go into `.megatron/proposals/`?).
2. What to do with the stray JS scripts. Should they be moved to `.megatron/scripts/` or `temp_scripts/`?
