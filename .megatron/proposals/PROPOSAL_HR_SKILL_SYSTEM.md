...
- **Mechanism:** After PM closes a GitHub Epic, an async background Map-Reduce task triggers HR to read the `.megatron/logs/` of the completed PRs.
...
4. **Deploy:** Deposit the newly generated `.md` file into `resources/plugins/skills/` (or `.megatron/registry/skills/`).
5. **Broadcast:** Update `.megatron/config/rules.md` or the `AGENTS.md` roster so the `pm` and `Master Agent` know this new capability exists.
