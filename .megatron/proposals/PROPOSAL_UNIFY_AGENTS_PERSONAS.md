...
| T2 | Role | Curated role template with principles | `megatron-plugin/roles/` | Stateless (read-only) | architect.md with core_principles, constraints |
| T1 | Agent | Role + project memory & context | `.megatron/agents/` | Stateful (knowledge, experience) | architect.md with 3-month project history |
...
| Directory: plugin | `megatron-plugin/agents/` | `megatron-plugin/roles/` (pure templates) |
| Directory: project| `.megatron/personas/` | `.megatron/agents/` (stateful entities) |

### The Lifecycle Flow:
```
User: "Find an architect to review this"

1. Check .megatron/agents/architect.md     → Found? → T1 Agent (has memory & project experience)
...
- All state management must flow through the blackboard (.megatron/)
...
role: architect
tier: T1
role_source: megatron-plugin/roles/architect.md
