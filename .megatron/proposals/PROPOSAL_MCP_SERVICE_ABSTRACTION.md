...
- **The Blackboard** (shared task state, T1 project memory, `.megatron/` directory).
...
We propose creating an internal Abstraction Layer inside the Megatron MCP Server. The AI Agents will **only** call standardized MCP tools. The MCP server will handle the underlying complex operations.

### A. Blackboard Abstraction (`mcp-blackboard`)
Instead of an Agent running raw bash commands like `echo "..." >> .megatron/agents/architect.md`, it uses MCP tools:
- `blackboard_read(key)`
