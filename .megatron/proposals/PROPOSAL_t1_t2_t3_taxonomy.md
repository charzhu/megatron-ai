# PROPOSAL: Mathematical Taxonomy of Megatron Swarm Tiers (T1, T2, T3)

## Pre-Requisite Context:
In our Multi-Agent Orchestrator framework (Megatron), we spawn headless AI CLI tools (like Claude Code, Copilot CLI) into specialized Sub-Agents doing tasks autonomously.

## The Proposal
We are formally defining the taxonomy of the Megatron Swarm registry as a mathematical composition of capabilities, moving away from purely abstract naming to actual system resource composition:
...
1. **T3 (Computational Resource Pool)**: Represents the raw hardware/engine configuration.
   ...
   - Persistence: Registered in `.megatron/registry/available-agents.json`.
...
2. **T2 (Role Templates)**: Represents a pre-defined persona instruction and workflow added onto a raw T3 compute layer. 
   ...
   - Persistence: Read-only templates stored in `.megatron/roles/*.md`.
...
3. **T1 (Local Session Agents)**: Represents a fully instantiated, stateful worker that actively retains memory within a discrete project.
   ...
   - Persistence: Stateful markdown files with YAML frontmatter pointing to UUIDs in `.megatron/agents/*.md`.
