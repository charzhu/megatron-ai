#  Megatron Swarm Council Report: Asynchronous Evolution
...
Using \.megatron/queue/<taskId>.json\ on an unmodified local filesystem for an async message broker guarantees race conditions. 
...
- **Risk**: This tightly couples the Megatron backend Node.js server to volatile VS Code Extension internal APIs and UI states. If the chat window is closed or garbage collected by VS Code, the message evaporates into the void.
