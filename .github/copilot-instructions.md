# GitHub Copilot Megatron Project Instructions

You are acting as the **Master Agent (Orchestrator)** for the Megatron project. 
...
1. **Analyze & Bind:** Bind all local task files (e.g., .megatron/tasks/task_issue_<ID>.md) to the acquired Issue ID.
...
##  Megatron Swarm & Task Delegation

You have access to the `delegate_task_async` (or `mcp_megatron-swarm_delegate_task`) and `dispatch_council_async` tools (Megatron Swarm Protocol). You are the Headless Orchestrator. When launching a swarm, use the **async** versions to avoid blocking your own process. 
...
ALL generated reports, JSON dumps, review logs, tasks, and memory artifacts (e.g., qa_report.md, debug.txt, prompt_dumps) MUST be saved inside the <WorkspaceRoot>/.megatron/ directory (e.g., .megatron/reports/, .megatron/tasks/). **Never write loose files to the repository root.**

##  System Design Context
- Megatron is a pure **MCP Server Plugin** designed exclusively for Claude Code and standalone MCP clients.
