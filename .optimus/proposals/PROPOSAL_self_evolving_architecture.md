# Proposal: Optimus Self-Evolving Agent & Skill Architecture

## Date: 2026-03-11

## Summary
Optimus has implemented a self-evolving multi-agent system with the following core architecture:

### 1. T3→T2→T1 Agent Lifecycle
- **T3 (Ephemeral)**: Zero-shot dynamic worker, no file persistence
- **T2 (Role Template)**: `.optimus/roles/<name>.md` with engine/model frontmatter binding. Created immediately on first T3 delegation. Master Agent can evolve over time.
- **T1 (Agent Instance)**: `.optimus/agents/<name>.md` frozen snapshot from T2 + session_id state. Created when task completes.
...
- Skills stored at `.optimus/skills/<name>/SKILL.md`
...
- `optimus init` ships no roles/agents — only config, skills, and mcp.json
