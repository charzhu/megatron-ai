...
## Background
Currently, Optimus uses an Agent Roster composed of T1 (local), T2 (project template), and T3 (dynamic outsourcing) roles. However, there is no formalized mechanism to capture successful, frequently used T3 roles and formally crystallize them into T2 (Project Default) templates (.optimus/roles/).

## Goal
Design a system to analyze T3 dynamically spawned workers, evaluate their performance or utility, and automatically or semi-automatically generate .md role templates under .optimus/roles/ so they become T2 experts.
