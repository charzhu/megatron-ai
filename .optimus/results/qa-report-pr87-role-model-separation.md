...
| Inline `--mcp-config` JSON is valid | Valid JSON with `mcpServers.spartan-swarm` | Lines 91-103: `JSON.stringify({mcpServers:{'spartan-swarm':{command:'node',args:[absolutePath],env:...}}})` — valid MCP config format | **PASS** |
| Absolute paths used | `path.join()` for all paths | `mcpServerJs = path.join(workspacePath, 'optimus-plugin', 'dist', 'mcp-server.js')` and `path.join(workspacePath, '.env')` | **PASS** |
