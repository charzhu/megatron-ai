# Blueprint: Migrating Megatron AI to Native CLI Session Architectures

## 1. The Core Realization
The original attempt in Megatron AI to establish a persistent daemon/REPL by spawning interactive interactive instances of `claude` and `github-copilot-cli` (via standard text I/O pipes) was fundamentally flawed. 
...
## 3. Refactoring Roadmap for Megatron AI

To fully transition Megatron AI into a pure "PM Agent Orchestrator", the following architectural shifts must occur:
...
When invoked by Megatron, execute:
```bash
claude -p "<latest_instruction_from_PM>" \
  --session-id "<megatron_task_uuid>" \
  --input-format stream-json \
  --output-format stream-json \
  --verbose \
  --dangerously-skip-permissions
```
*   *Note*: The CLI output must be mapped from Claude's internal JSON stream types (`message_start`, `chunk`, `tool_started`, `result`) to Megatron's `ChatViewProvider` UI update callbacks.

#### For `GitHubCopilotAdapter.ts`
When invoked by Megatron, execute:
```bash
copilot -p "<latest_instruction_from_PM>" \
  --resume "<megatron_task_uuid>" \
  --output-format json \
  --stream on \
  --allow-all-tools
```
...
1. **The PM Agent (Megatron VS Code Host)**: Reads constraints, maintains global Markdown documents (e.g. `task.md`), allocates a session UUID, and triggers the Workers.
...
Instead of spawning CLI processes for every cognitive task (like breaking down user requirements, acting as the PM, or evaluating code quality), Megatron AI will leverage the **VS Code Native Language Model API** (`vscode.lm`) for its central orchestrator ("The Brain").
...
for await (const fragment of chatResponse.text) {
    // Stream directly to the Megatron UI / PRD.md
}
```
...
**Mapping to Megatron AI:**
*   **The Blackboard**: The `.megatron/` directory in the user's workspace (acting as our shared state).
*   **Knowledge Source 1 (The Brain)**: The `vscode.lm` driven PM Agent that analyzes requirements and writes the initial plan.
*   **Knowledge Source 2 (The Hands)**: The CLI-driven Coding Agents (`claude --session-id`) that read the `.megatron/TODO.md` and execute changes.
*   **The Controller (Orchestrator)**: The Megatron VS Code Extension itself, which monitors the file changes, manages human approval flows, and spawns the CLI processes.
...
2. **Markdown Contracts (Protocol)**: Communication occurs structurally via files like `.megatron/PRD.md` and `.megatron/TODO.md`. 
   * The PM writes `- [ ] Task 1`.
   * The Worker completes the task and updates it to `- [x] Task 1`.
3. **Human-in-the-Loop (HITL)**: Before the CLI Workers are spawned by the Orchestrator, the generated `.megatron/TODO.md` is presented to the user. The human acts as the supreme project manager—approving or tweaking the Markdown file. Only upon human approval does the Orchestrator read the file and dispatch the Worker.
4. **Error Feedback Loop (Blockers)**: If a CLI Worker reaches a dead end (e.g., repeating API errors or dependency conflicts), it is instructed to halt execution (`exit 0`) and record the obstacle in a `.megatron/BLOCKERS.md` file. The Orchestrator monitors this outcome and routes the blocker back to the PM Agent (`vscode.lm`) to either re-plan or solicit human intervention.
...
To evolve the current Megatron AI project into this target architecture, we will use a gradual, "Dogfooding" approach (using Megatron to build Megatron). Every phase must be independently usable and testable. 
...
*   **Action:** 
    1. Create the `.megatron/` directory in the project root.
    2. Define a `.megatron/rules.md` (the protocol for the Worker).
    3. The human talks to their existing Copilot/Claude Chat interface (acting as the "PM"), asks it to break down tasks, and instructs it to output a `.megatron/TODO.md`.
...
*   **Action:** After Phase 1 generates `.megatron/TODO.md`, the human manually runs the CLI worker in the VS Code terminal: 
    ```bash
    claude -p "Read .megatron/TODO.md, execute the first unfinished task. When done, change '- [ ]' to '- [x]'." --session-id megatron-v2-dev
    ```
...
### Phase 3: Megatron as the Execution Orchestrator
**Goal:** Replace the legacy, hanging `persistent` REPL logic with clean, automated background `spawn` calls. Megatron becomes a dedicated "Task Execution Engine", ignorant of complex NLP or chat histories.
*   **Action:** 
    1. Tear down old PTY/daemon hacks in `ClaudeCodeAdapter.ts`. 
    2. Implement a simple one-shot runner `runTask(uuid)`.
    3. Add a simple **"Execute Next Task"** button in Megatron. Clicking this button merely reads `.megatron/TODO.md`, formats a static prompt to the CLI, and spawns the subprocess.
*   **Value:** Agent-to-Agent (A2A) is fully connected via UI. The Megatron extension's codebase is heavily simplified, focusing on IO orchestration rather than prompt-engineering.
...
*   **Action:** Implement the Orchestrator loop in the extension host. 
    ...
    2. Extension reads `.megatron/TODO.md` and dispatches Worker.
    3. Monitor `exit 0`, check if task was checked `[x]`, move to the next task automatically.
    4. Implement `vscode.lm` for automated replanning if `.megatron/BLOCKERS.md` is updated by a failing worker.
...
*   **Action:** 
    ...
    3. Inject strict `role_prompt`s into the adapter instructing the agents to dump their findings solely into `.megatron/reviews/<role>.md`.
