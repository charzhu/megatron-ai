...
/**
 * `megatron upgrade` — Force-overwrite skills, canonical roles, and config
...
module.exports = function upgrade() {
  ...
  // Pre-check: .megatron/ must exist
  if (!fs.existsSync(megatronDir)) {
    console.error('Error: No .megatron/ directory found. Run \'megatron init\' first.');
    process.exit(1);
  }
  ...
  console.log(`\n🔄 Upgrading to megatron-swarm v${version}...\n`);
  ...
  if (fs.existsSync(skillsSrc)) {
    ...
    skillCount = copyDirForceOverwrite(skillsSrc, path.join(megatronDir, 'skills'));
  }
  ...
  if (fs.existsSync(rolesSrc)) {
    ...
    roleCount = copyDirForceOverwrite(rolesSrc, path.join(megatronDir, 'roles'));
  }
  ...
  if (fs.existsSync(configSrc)) {
    ...
    configCount = mergeConfigFiles(configSrc, path.join(megatronDir, 'config'));
  }
  ...
  if (fs.existsSync(systemSrc)) {
    ...
    const destSystem = path.join(megatronDir, 'system');
  ...
  // 6. .vscode/mcp.json: Re-generate megatron-swarm entry (always overwrite)
  ...
  if (fs.existsSync(vscodeMcpPath)) {
    try {
      ...
      console.log('\n🔌 Updated megatron-swarm entry in .vscode/mcp.json');
    } catch (e) {
