...
PM writes a requirements doc at `.megatron/tasks/requirements_<feature>.md`
...
- You can also check the `MEGATRON_TRACKING_ISSUE` environment variable as a fallback
...
```
dispatch_council(
  proposal_path: ".megatron/tasks/requirements_<feature>.md",
...
```
dispatch_council(
  proposal_path: ".megatron/tasks/requirements_<feature>.md",  // enriched with project context
...
Before delegating to Dev, PM MUST verify that every skill listed in `required_skills` actually exists at `.megatron/skills/<name>/SKILL.md`. If a skill is missing:
...
delegate_task(
  ...
  parent_issue_number: <issue_number>,
  output_path: ".megatron/reports/implementation_<feature>.md",
  workspace_path: "<project root>",
...
```
dispatch_council(
  proposal_path: ".megatron/reports/implementation_<feature>.md",
...
Example: `megatron upgrade` must be tested against a `.megatron/config/vcs.json` with real org/project values.
